...
server.registerTool(
  ...
  {
    description:
      "Modify the design. Sends a prompt you expect to change the served HTML (e.g., 'create a login screen', 'add a Remember-me checkbox'). Waits for HTML to change and stabilize. Returns slim metadata — NOT inline HTML (written to disk at htmlPath). Also diffs the file list so you know what Claude created/removed.\n\nAuto-appended to every prompt: an instruction to keep all generated files at the project root (no subfolders). The live MCP's file-list scrape is flat-only; subfolder-nested files are invisible until you call designer_handoff. If you genuinely need nested layouts, explicitly contradict this in your prompt and rely on handoff for file access.\n\nKey return fields:\n- done.failureMode: null | 'timeout' | 'unstable' | 'no_change' (no_change means Claude replied text-only — did you want designer_ask?)\n- newFiles / removedFiles: diff vs pre-send\n- activeFile: what's currently rendered\n- htmlPath / screenshotPath: read these if you need the content\n- chatReply: Claude's commentary",
    inputSchema: {
